...
def jacobian_ik6_step(model: mujoco.MjModel, data: mujoco.MjData, site_id: int, robot_idx: int,
                      v_des: np.ndarray, w_des: np.ndarray, dt: float, damping: float = 0.1) -> np.ndarray:
    """Compute dq for desired 6D twist (v_des linear, w_des angular)."""
    nv = model.nv
