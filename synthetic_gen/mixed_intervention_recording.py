...
from synthetic_gen.intervention_policies import (  # type: ignore
    AutoPickupPolicy,
    AutoGrabPolicy,
)
from synthetic_gen.intervention_guis import (  # type: ignore
    create_mixed_gui,
    create_mixed_gui_autograb,
)
...
class AutoGrabPolicy:
    ...
    graspable distance from the object, then pulls the object upward toward a
    global target z-height.

    Phases:
      0: Approach until EE-object distance < grasp_distance.
      1: Close gripper while maintaining proximity.
      2: Lift object so its z approaches target_object_z.
    ...
    def step(self) -> None:
        ...
        if self.phase == 1:
            # Keep approaching slightly if we drift away.
            if dist > self.grasp_distance:
                dpos = self._clamped_delta(
                    ee_pos,
                    obj_pos,
        ...
        if self.phase == 2:
            ...
            # Always move toward the global target; once close in Z, stop.
            dpos = self._clamped_delta(
                ee_pos,
                target,
                0.5 * self.controller.pos_step,
